...
def graph_print_one_bar(divide, color):
...
def graph(sale, exp):
    ...
    divide = sale // 40
    graph_print_one_bar(divide, 'black')
    ...
    divide = exp // 40
    graph_print_one_bar(divide, 'red')
...
def graph_headings(low_sales_mth, high_sales_mth, low_exp_mth, high_exp_mth, avg_sales, avg_expenditure,
                   total_sales_str, total_expenditure_str, yearly_profit_str):
    ...
    turtle.goto(0, -260)
    turtle.write(total_sales_str)
    turtle.goto(0, -280)
    turtle.write(total_expenditure_str)

    turtle.goto(0, -320)
    turtle.write(yearly_profit_str)
...
avg_sales = avg_expenditure = yearly_profit = 0

low_sales_month = high_sales_month = ""
low_exp_month = high_exp_month = ""
lowest_exp_month = highest_exp_month = ""
total_sales_str = total_expenditure_str = ""
...
total_sales_str = f"Total sales: £{total_sales:,d}"
total_expenditure_str = f"Total expenditure: £{total_expenditure:,d}"
yearly_profit_str = f"2018 Profit: £{yearly_profit:,d}"

graph_headings(low_sales_mth, high_sales_mth, low_exp_mth, high_exp_mth, avg_sales, avg_expenditure, total_sales_str,
               total_expenditure_str, yearly_profit_str)
